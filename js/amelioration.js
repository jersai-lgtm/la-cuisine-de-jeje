  ...
  if (!modal) {
    ...
    modal.innerHTML =
      '<div style="background:#211e26;border:1px solid rgba(255,255,255,.12);border-radius:18px;padding:18px;max-width:440px;width:100%;box-sizing:border-box">' +
        ...
        '<label style="color:var(--text-2);font-size:12px;display:block;margin:0 0 4px">Sujet concerné <span style="color:#88858f">(appli, recette, photo… — facultatif)</span></label>' +
        '<input id="amelioration-sujet" type="text" maxlength="120" placeholder="Ex : l\'appli en général, la recette Tarte Tatin, une photo…" style="width:100%;box-sizing:border-box;background:var(--surface-1);color:#fff;border:1px solid rgba(255,255,255,.15);border-radius:12px;padding:10px;font-size:14px;margin:0 0 12px">' +
        '<label style="color:var(--text-2);font-size:12px;display:block;margin:0 0 4px">Ton message</label>' +
        '<textarea id="amelioration-texte" maxlength="800" rows="4" placeholder="Décris ce qu\'il faudrait modifier, ajouter ou supprimer…" oninput="majCompteurAmelioration()" style="width:100%;box-sizing:border-box;background:var(--surface-1);color:#fff;border:1px solid rgba(255,255,255,.15);border-radius:12px;padding:10px;font-size:14px;resize:vertical"></textarea>' +
