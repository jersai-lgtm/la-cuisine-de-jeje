...
const RECETTES_NON_REPAS = new Set([
  ...
  "jusPastequeMenthe","limonademaison","madeleine","margarita","millefeuille","mimosa",
...
function chargerAccueilTopMois() {
  ...
  const header = titre ? titre.closest(".accueil-bloc-header") : null;
  if (titre) {
    const label = `Le top de ${NOMS_MOIS_FR[mois]}`;
    const saisonActuelle = (typeof getSaisonActuelle === "function") ? getSaisonActuelle() : "printemps";
    const banniere = (typeof BANNIERE_SAISON !== "undefined") ? BANNIERE_SAISON[saisonActuelle] : null;
    if (header && banniere) {
      // Bandeau colore (harmonise avec le CTA swipe), degrade de la saison en cours
      header.style.background = `linear-gradient(95deg, ${banniere.grad[0]}, ${banniere.grad[1]})`;
      header.style.borderRadius = "14px";
      header.style.padding = "12px 14px";
      header.style.marginBottom = "12px";
      titre.style.color = banniere.texte;
    }
...
// Bandeau « Le top du mois » : dégradé + couleur de texte contrastée par saison
const BANNIERE_SAISON = {
  printemps: { grad: ["#3b8f5f", "#7cc576"], texte: "#0f2e18" },
  ete:       { grad: ["#e0a82e", "#f0c85a"], texte: "#3d2b02" },
  automne:   { grad: ["#a05a2c", "#c9895a"], texte: "#fff4e8" },
  hiver:     { grad: ["#3f7fc4", "#7fb3e8"], texte: "#0a1d3d" },
};
...
// INGREDIENTS_LABELS a été déplacé dans ingredients_prix.js (rangé à côté des prix).
...
function calculer(recetteArg, personnesArg) {
  ...
  const cocktailsTables = {
    ...
    "jusPastequeMenthe": { table: "tableauJusPasteque",   fn: htmlTableauJusPastequeColonnes,  label: "verre" },
  ...
  if (tableauKey) {
    ...
    if (ligne) {
      document.getElementById("resultat").innerHTML =
        `<h3>${window.LANG==="en"?"For":"Pour"} ${personnes} ${getUniteRecette(recette, personnes)}</h3>` +
  ...
  let html = `<h3>${window.LANG==="en"?"For":"Pour"} ${personnes} ${getUniteRecette(recette, personnes)}</h3>`;
...
async function envoyerQuestionClaude(nom) {
  ...
  // Sécurité : l'assistant passe par un proxy protégé. Il faut être connecté
  // (le proxy exige un jeton Firebase valide → empêche l'abus de la clé API).
  if (!window.currentUser) {
    if (typeof afficherToast === "function") afficherToast("Connecte-toi pour utiliser l'assistant 🤖");
...
Réponds en français.`;
  ...
  try {
    // Proxy Cloudflare Worker (évite le blocage CORS + protège la clé API).
    // On envoie le jeton d'identité Firebase : le Worker le vérifie et applique
    // un quota par utilisateur (voir worker/index.js).
    ...
    // Erreurs spécifiques au proxy protégé (avant de parser le JSON).
    if (response.status === 401) {
      document.getElementById(loadingId)?.remove();
      afficherMessageClaude(nom, "assistant", "Ta session a expiré — reconnecte-toi puis réessaie 🙏");
      return;
    }
    if (response.status === 429) {
      document.getElementById(loadingId)?.remove();
      afficherMessageClaude(nom, "assistant", "Tu as posé beaucoup de questions 😅 Réessaie un peu plus tard.");
      return;
    }
